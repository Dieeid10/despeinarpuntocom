...
def get_clients():
    return database['select_case'](
        'SELECT id, nombre, apellido, documento, email, telefono, fecha_nacimiento, direccion FROM clientes ORDER BY apellido'
    )


def get_client_by_id(client_id: UUID):
    return database['select_case'](
        'SELECT id, nombre, apellido, documento, email, telefono, fecha_nacimiento, direccion FROM clientes WHERE id = ?',
        (str(client_id),)
    )
...
def update_client( client_id: UUID, nombre: str, apellido: str, documento: str, email: str, telefono: str, fecha_nacimiento: str, direccion: str ):
    database['update_case'](
        ...
        (nombre, apellido, documento, email, telefono, fecha_nacimiento, direccion, str(client_id))
    )
